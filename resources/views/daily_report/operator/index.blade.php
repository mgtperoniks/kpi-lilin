        ...
        <div class="flex flex-col gap-3">
            @foreach($dates as $row)
                <a href="{{ route('daily_report.operator.show', $row->production_date) }}"
                    class="group bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md hover:border-green-400 transition-all duration-200 flex items-center justify-between">

                    <div class="flex items-center gap-4">
                        <div
                            class="flex items-center justify-center w-12 h-12 rounded-full bg-green-50 text-green-600 group-hover:bg-green-600 group-hover:text-white transition-colors shrink-0">
                            <span
                                class="text-xs font-bold uppercase">{{ \Carbon\Carbon::parse($row->production_date)->locale('id')->isoFormat('MMM') }}</span>
                        </div>
                        <div>
                            <h3 class="text-lg font-bold text-gray-800 group-hover:text-green-700">
